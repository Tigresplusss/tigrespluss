...
require_once 'conn.php'; // Incluye el archivo para la conexión a la base de datos
require_once 'login.php'; // Incluye el archivo para manejar la sesión de usuario
...
// Verifica si se ha recibido un parámetro 'type' e 'id' en la solicitud
if (isset($_GET['type'], $_GET['id'])) {
    $type = $_GET['type']; // Tipo de contenido (e.g., 'pelicula')
    $idpelicula  = (int)$_GET['id']; // ID de la película, convertido a entero

    // Maneja diferentes tipos de solicitudes según el valor de 'type'
    switch ($type) {
        case 'pelicula':
            // Maneja la acción de "like" para una película
            if (isset($_POST['liked'])) {
                $pelid = $_POST['pelid']; // ID de la película a la que se le da like
                // Consulta para obtener el número actual de likes
                $result = mysqli_query($conn, "SELECT * FROM pelicula WHERE id=$pelid");
                $row = mysqli_fetch_array($result);
                $n = $row['likes']; // Número actual de likes
        
                // Inserta un nuevo like en la tabla 'likes'
                mysqli_query($conn, "INSERT INTO likes (pelid, userid) VALUES ($pelid,{$_SESSION['user']})");
                // Actualiza el conteo de likes en la tabla 'pelicula'
                mysqli_query($conn, "UPDATE pelicula SET likes=$n+1 WHERE id=$pelid");
        
                echo $n+1; // Devuelve el nuevo número de likes
                exit();
            }

            // Maneja la acción de "unlike" para una película
            if (isset($_POST['unliked'])) {
                $pelid = $_POST['pelid']; // ID de la película a la que se le quita el like
                // Consulta para obtener el número actual de likes
                $result = mysqli_query($conn, "SELECT * FROM pelicula WHERE id=$pelid");
                $row = mysqli_fetch_array($result);
                $n = $row['likes']; // Número actual de likes
        
                // Elimina el like de la tabla 'likes'
            ...
            // Comentarios de depuración (comentados)
...
<body>
    ...
    <!-- Itera sobre cada elemento en el array $generos -->
    <li>
        <!-- Crea un elemento de lista con un enlace -->
        <a class="dropdown-item" 
           href="genero-menu-image.php?type=genero&idgenero=<?php echo $genero->idgenero; ?>">
           <!-- Enlace que redirige a la página de género con el ID del género como parámetro -->
           <?php echo $genero->nombre_genero; ?>
           <!-- Muestra el nombre del género en el enlace -->
        </a>
    </li>
<?php endforeach; ?>
                ...
                <?php if ($_SESSION['user'] == 0 ) : ?>
                        <form class="d-flex">
                            <a href="logearse.php" class="btn btn-outline-warning d-none d-md-inline-block " type=button>Iniciar Sesion</a>
    
                        </form>
                <?php else: ?>
                    <form class="d-flex">
                        <a href="logout.php" class="btn btn-outline-warning d-none d-md-inline-block " type=button>Cerrar Sesion</a>
    ...
    <?php if ($_SESSION['user'] == 0 ) : ?>
        <!-- Si el usuario no está logueado, mostrar un botón que redirige a la página de inicio de sesión para dar me gusta -->
        <a role="button" class="btn btn-primary mt-3" href="logearse.php">Me gusta <i class="bi bi-hand-thumbs-up-fill"></i> </a>
    <?php else: ?>
        <?php 
        // Si el usuario está logueado, verificar si ya ha dado me gusta a esta película
        $results = mysqli_query($conn, "SELECT * FROM likes WHERE userid={$_SESSION['user']} AND pelid=".$row['id']."");

        if (mysqli_num_rows($results) == 1 ): ?>
            <!-- El usuario ya ha dado me gusta a esta película -->
            <span class="unlike fa fa-thumbs-up" data-id="<?php echo $row['id']; ?>"></span> 
            <span class="like hide fa fa-thumbs-o-up" data-id="<?php echo $row['id']; ?>"></span> 
        <?php else: ?>
            <!-- El usuario aún no ha dado me gusta a esta película -->
                        ...
                        <div class="container px-4">
                            <div class="row gx-4 justify-content-center">
                                <div class="col-lg-8">
                                        ...
                                        <?php if ($_SESSION['user'] == 0 ) : ?>
    <!-- Si el usuario no está logueado, mostrar un botón que redirige a la página de inicio de sesión -->
    <a role="button" class="btn btn-dark" href="logearse.php"><i class="bi bi-send-fill"></i> Comentar</a>
                        
<?php else: ?>
    <!-- Si el usuario está logueado, mostrar un botón de tipo submit para enviar el comentario -->
...
<?php endif ?>
    ...
    # Si deseas depurar, puedes descomentar la línea siguiente para ver el ID de la película
    #echo $idpelicula;

    # Ejecutar una consulta para obtener todos los comentarios para la película con el ID especificado
    $resultado = $conn->query("SELECT * FROM comentarios 
    LEFT JOIN user
    ON comentarios.idusuario = user.userid  
    WHERE idpelicula = {$idpelicula}");

    # Recorrer todos los comentarios obtenidos
    while ($comentario = mysqli_fetch_object($resultado)) {
?>

    <!-- Mostrar el nombre de usuario y la fecha del comentario -->
...
?>
    ...
    <div class="row mt-5">
        <footer>
            <p class="mt-3">Todos los derechos reservados | © Los Tigres+</p>
